...
export function fileRoutes(authService: AuthService, uploadDir: string) {
  const router = Router();
  ...
  function resolveSafePath(relativePath: string): string {
    ...
    if (!resolved.startsWith(rootDir)) {
      throw new Error(`Permission denied: path "${relativePath}" escapes uploads directory.`);
    }
  ...
  router.get('/ls', authorizePermission(authService, 'read'), async (req: Request, res: Response) => {
    try {
  ...
  router.get('/readFile', authorizePermission(authService, 'read'), (req: Request, res: Response) => {
