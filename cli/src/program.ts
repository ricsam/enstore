...
const mkdirCmd = new Command("mkdir")
  .argument("<remoteDir>", "Directory path to create on the EnStore server")
  .option("-r, --recursive", "Create parent directories if needed")
  .option(
    "--mode <mode>",
    "File mode (octal). If a string is passed, it is parsed as an octal integer.",
  )
  .description("Create a new directory")
  .action(async (remoteDir: string, options: MkdirOptions) => {
    const credentials = resolveCredentials();

    // Parse the `mode` string as octal, if provided
    let parsedMode: number | undefined;
    if (options.mode) {
      parsedMode = parseInt(options.mode, 8); // parse octal string -> number
      if (isNaN(parsedMode)) {
        console.error(
          `Invalid mode: "${options.mode}". Must be a valid octal string (e.g. "755", "644").`,
        );
        process.exit(1);
      }
    }
    try {
      // Send a POST request to /files/mkdir with { path: remoteDir }
      const resp = await axios.post(
        `${credentials.endpoint}/files/mkdir`,
        {
          options: {
        ...
        {
          params: { path: remoteDir },
          auth: credentials,
        },
      );
      // The server should respond with a JSON { message: '...' }
      console.log(resp.data.message || `Directory created: ${remoteDir}`);
    } catch (error: any) {
      console.error(
